...
public abstract class AbstractTmdbApi {
	...
	protected AbstractTmdbApi(TmdbApi tmdbApi) {
	...
	public <T> T mapJsonResultXrel(ApiUrlXrel apiUrl, Class<T> someClass) {
		return mapJsonResult(apiUrl, someClass, null);
	}
	...
	public <T> T mapJsonResult(ApiUrlXrel apiUrl, Class<T> someClass, String jsonBody) {
		try {
			return mapJsonResult(apiUrl, someClass, jsonBody, RequestMethod.GET);
		} catch (MalformedURLException e) {
			e.printStackTrace();
	...
	private static final Collection<Integer> SUCCESS_STATUS_CODES = Arrays.asList(1, // Success
	...
	public <T> T mapJsonResult(ApiUrl apiUrl, Class<T> someClass, String jsonBody, RequestMethod requestMethod)
		...
		String webpage;
		webpage = tmdbApi.requestWebPage(apiUrl, jsonBody, requestMethod);
		...
		try {
			...
			// work around the problem that there's no status code for suspected
			// spam names yet
			...
			// if null, the json response was not a error responseStatus code,
	...
	public <T> T mapJsonResult(ApiUrlXrel apiUrl, Class<T> someClass, String jsonBody, RequestMethod requestMethod)
			throws MalformedURLException {

		String webpage;
		webpage = tmdbApi.requestWebPageXrel(apiUrl, jsonBody, requestMethod);
